...
{
    $key = $row['fm_id'];
    //if (!$fm_id) $fm_id = $key; // wetoz : 전체보이기 위함
...
//include_once('./_head.php'); // wetoz
$board['gr_id']      = 'reserv';
$group['gr_subject'] = '예약';
$board['bo_subject'] = '자주하는 질문';

$page_link2 = G5_BBS_URL.'/faq.php';
...
if(is_file($skin_file)) {
    ...
    // wetoz
    if ($fm_id) {
        $sql_search_arr[] = " fm_id = '$fm_id' ";
    }
    if($stx) {
        switch ($sfl) {
            case "subject":
                $sql_search_arr[] = " INSTR(fa_subject, '$stx') > 0 ";
            break;
            case "content":
                $sql_search_arr[] = " INSTR(fa_content, '$stx') > 0 ";
            break;
            case "sc":
                $sql_search_arr[] = " ( INSTR(fa_subject, '$stx') > 0 or INSTR(fa_content, '$stx') > 0 ) ";
            break;
            default:
                $sql_search_arr[] = " ( INSTR(fa_subject, '$stx') > 0 or INSTR(fa_content, '$stx') > 0 ) ";
            break;
    ...
    $sql_search = '';
    if (!empty($sql_search_arr)) {
        $sql_search = " where ". join(" and ", $sql_search_arr);
    }
    ...
    */
    ...
    // wetoz : 페이징추가
    $write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './faq.php?fm_id='.$fm_id.$qstr.'&amp;page=');
